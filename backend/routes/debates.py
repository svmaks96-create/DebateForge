...
from models import ArgumentAnalysis, Debate, DebateAgent, Persona, Round
...
PRO_PREFIXES = ["A", "C", "E", "G"]
CON_PREFIXES = ["B", "D", "F", "H"]


@router.post("", response_model=DebateResponse, status_code=201)
async def create_debate(body: CreateDebateRequest, db: AsyncSession = Depends(get_db)):
    # Resolve format config
    if body.format_config:
        format_config = body.format_config
    else:
        fmt_name = body.format_name or "oxford"
    ...
    panel_config = body.panel_config or {
        "pro_count": len(body.agents.pro),
        "con_count": len(body.agents.con),
    }
    ...
    # Create agents
    for i, agent_input in enumerate(body.agents.pro):
        await _create_agent(db, debate.id, "pro", i, PRO_PREFIXES[i], agent_input)

    for i, agent_input in enumerate(body.agents.con):
        await _create_agent(db, debate.id, "con", i, CON_PREFIXES[i], agent_input)
...
async def _create_agent(db, debate_id, side, position, prefix, agent_input):
...
async def get_debate(debate_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    debate.rounds = sorted(debate.rounds, key=lambda r: r.round_number)
...
async def start_debate(debate_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    asyncio.create_task(run_debate(debate_id, async_session))
        ...
        "argument_analyses": [
            {
                ...
                "claim_score": float(a.claim_score) if a.claim_score else None,
                "grounds_score": float(a.grounds_score) if a.grounds_score else None,
                "warrant_score": float(a.warrant_score) if a.warrant_score else None,
                "backing_score": float(a.backing_score) if a.backing_score else None,
                "qualifier_score": float(a.qualifier_score) if a.qualifier_score else None,
                "overall_strength": float(a.overall_strength) if a.overall_strength else None,
